...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="es" className="h-full">
      <head>
        {/* Escudo anti-flash de starfields externos */}
...
@media (max-width:768px){
  ...
  .cloud-a{width:84vw}.cloud-c{width:92vw}.cloud-b{width:100vw}
        ...
        {/* ===== SCRIPT: Nubes orgánicas (visibilidad corregida) ===== */}
...
(function(){
  ...
  function makeNoise1(seed){
    ...
    return function(x){const X=Math.floor(x)|0, xf=x-X, u=fade(xf); const a=g[(X)&255], b=g[(X+1)&255]; return (a*xf)+((b*(xf-1)-(a*xf))*u);}
  }

  function drawPuff(ctx,x,y,r,bright){
    const g=ctx.createRadialGradient(x,y,0,x,y,r);
    // ↑ ALFAS SUBIDAS para que siempre se vean
    g.addColorStop(0.00,'rgba(255,255,255,'+(0.36+0.16*bright).toFixed(3)+')');
    g.addColorStop(0.45,'rgba(255,255,255,'+(0.22+0.10*bright).toFixed(3)+')');
  ...
  function cleanupAlpha(ctx){
    ...
    for(let i=0;i<d.length;i+=4){
      ...
      if(a<0.03){ d[i+3]=0; continue; }   // ← umbral más bajo
      d[i+3]=Math.max(0,Math.min(255,Math.floor(a*255)));
    }
    ctx.putImageData(img,0,0);
  }

  function blurCopy(srcCtx,dstCtx){
    const W=srcCtx.canvas.width,H=srcCtx.canvas.height;
    const tmp=document.createElement('canvas'); tmp.width=W; tmp.height=H;
    const tx=tmp.getContext('2d'); tx.drawImage(srcCtx.canvas,0,0);
    dstCtx.clearRect(0,0,W,H);
    dstCtx.filter='blur(0.4px)'; dstCtx.drawImage(tmp,0,0); dstCtx.filter='none';
  }
  ...
  function paintCloud(canvas,variant){
    try{
      ...
      const COUNT = (variant===0?120:variant===1?140:130);
      const spineY = (variant===0?0.56:variant===1?0.58:0.57);
      const ampSin1 = 0.06, ampSin2 = 0.03, ampNoise = 0.035;
      const baseR  = H * (variant===0?0.060:variant===1?0.062:0.061);
      ...
      while(pts.length<COUNT && tries<maxTries){
        tries++;
        const u = Math.min(1, Math.max(0, Math.pow(rng(),0.85)));
        const x = (margin + u*spread) * W + (rng()-0.5)*W*0.01;
        const t = (x/W - margin)/spread;
        const yBase = spineY
      ...
      // Blur seguro (sin dibujar el mismo canvas sobre sí mismo)
      blurCopy(fx, fx);
