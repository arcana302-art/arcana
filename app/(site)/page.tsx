...
import Hero from "../components/Hero"; // ⬅️ IMPORTANTE: nuevo hero
...
export default function Page() {
  ...
  return (
    <>
      {/* ===== HERO NUEVO (místico-poético, ES por defecto) ===== */}
      <Hero lang="es" />
            ...
            <form className="mt-4 grid gap-4 md:grid-cols-2" onSubmit={onSubmitAgenda}>
              ...
              <div className="grid gap-1">
                ...
                <select className="rounded border border-white/15 bg-white/5 px-3 py-2 text-white" name="categoria">
                  {categorias.map(c => (<option key={c} className="bg-[var(--arcana-bg)] text-white">{c}</option>))}
                </select>
              </div>
              <div className="grid gap-1">
